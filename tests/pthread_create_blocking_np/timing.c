...
#define WAIT_MAIN_AFTER_BLOCK 0x02

static struct testcase {
	int options;
} tests[] = {
	{WAIT_THREAD_AFTER_START},
	{WAIT_MAIN_AFTER_BLOCK},
	{WAIT_THREAD_AFTER_START | WAIT_MAIN_AFTER_BLOCK},
	{0}
};
...
runner(int options1, int options2)
{
	...
	/* Wait if the test calls for it. */
	if (info1.options & WAIT_MAIN_AFTER_BLOCK)
		wait_ms(WAIT_MS);
	if (info2.options & WAIT_MAIN_AFTER_BLOCK)
		wait_ms(WAIT_MS);
